...
function useColumns<T>(
  ...
  const [tableColumns, setTableColumns] = React.useState<TableColumn<T>[]>(() =>
    ...
    (e: React.DragEvent<HTMLDivElement>) => {
      ...
      if (id) {
        sourceColumnId.current =
          tableColumns[findColumnIndexById(tableColumns, id)]?.id?.toString() ||
          '';
  ...
  const handleDragEnter = React.useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      ...
      if (id && sourceColumnId.current && id !== sourceColumnId.current) {
        const selectedColIndex = findColumnIndexById(
          tableColumns,
          sourceColumnId.current
        );
  ...
  const defaultSortColumn = React.useMemo(
    () =>
      tableColumns[
        findColumnIndexById(tableColumns, defaultSortFieldId?.toString())
      ] || {},
