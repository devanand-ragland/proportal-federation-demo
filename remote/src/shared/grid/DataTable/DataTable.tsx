...
function DataTable<T>(props: TableProps<T>): JSX.Element {
  ...
  const currentTheme = React.useMemo(() => createStyles(customStyles, theme), [
    customStyles,
    theme,
  ]);
  ...
  const handleRowClicked = React.useCallback((row, e) => onRowClicked(row, e), [
    onRowClicked,
  ]);
      ...
      <ResponsiveWrapper
        ...
        <Wrapper>
          ...
          <Table disabled={disabled} className='rdt_Table' role='table'>
            {showTableHead() && (
              <Head
                className='rdt_TableHead tblCustomHead'
                role='rowgroup'
                fixedHeader={fixedHeader}>
                <HeadRow className='rdt_TableHeadRow' role='row' dense={dense}>
                  ...
                  {expandableRows && !expandableRowsHideExpander && (
                    <ColumnExpander className='expanderCls' />
                  )}
            ...
            {!progressPending && sortedData.length > 0 && (
              <Body className='rdt_TableBody' role='rowgroup'>
                {tableRows.map((row, i) => {
                  const key = prop(row as TableRow, keyField) as
                    | string
                    | number;
